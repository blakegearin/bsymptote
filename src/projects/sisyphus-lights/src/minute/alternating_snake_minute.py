#!/usr/bin/env python3
# Name: Alternating Snake Minute
# Author: Blake Gearin (blakegearin@pm.me)
#
# Description: Snake alternating its color every minute
...
def init(strip, table_values):
    global transition, time_start, is_primary, last_active, current_primary, current_secondary
    time_start = 0
    transition = 0
...
def update(strip, table_values):
    ...
    if (now.second == 0) and (now.minute != last_active):
        is_primary = (not is_primary)
        last_active = now.minute

    current_color = current_primary if is_primary else current_secondary
    ...
    for x in range(s_start, s_end):
        pos = x % led_count
        strip.setPixelColor(pos, current_color)
